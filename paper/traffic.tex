...
\begin{algorithm2e}
\KwData{$\mathtt{D}^{\downarrow}[u]$: tentative distance from $u$ to $t$ as computed by Dijkstra's algorithm on $\gchd$}
...
Lazy RPHAST~\cite{strasser_et_al:LIPIcs.SEA.2021.6} is a CH-based algorithm to quickly compute distances from many nodes to a single target.
...
Our implementation extends on the original CH-Potentials algorithm by additionally storing parent pointers for path unpacking.
...
Query algorithms can be reused from CH without any modification.
...
According to~\cite{dss-tarrn-18}, the UBS computation is one of the bottlenecks of the IDB approach.
...
The authors of this paper state that it would be ideal to check the UBS in time proportional to the length of the path and a few shortest path queries but due to the lack of such an option they resort to approximations.
...
We abort queries if the algorithm after 250 iterations does not have found a path with $\ubs < (1+\epsilon)$.
...
\caption{
Running times of different UBS checking algorithms for paths encountered by IDB when answering queries of different ranks with $\epsilon = 0.2$.
...
\centering
\caption{
Average performance of our implementations of IDB and IPB for different values of $\epsilon$ with 1h queries on OSM Europe with synthetic live traffic.
...
All other columns indicate average values over all queries, including the ones terminated after 250 iterations.
}\label{tab:epsilon}
\input{table/epsilon.tex}
\end{table}

Next, we evaluate the performance of iterative path blocking and iterative detour blocking for different choices of epsilon.
This experiment was also performed in~\cite{dss-tarrn-18}.
We replicate the setup to the best of our abilities to allow for comparison of the different IDB implementations.
...
% less? blocked paths for small epsilons, more? for larger epsilons
Our implementation of IDB achieves running times two to three orders of magnitude faster than the baseline in~\cite{dss-tarrn-18}.
...
% CRP not as effective with many blocked paths?

In comparison to IDB, IPB needs more iterations and blocks significantly more paths.
% goes in hand with more failed queries
Conversely, it also finds shorter paths.
The qualitative differences between IDB and IPB become more pronounced the smaller $\epsilon$ becomes.
Interestingly, IDB is only faster than IPB for $\epsilon \geq 0.1$.
The reason for this is that the running time of the A* search depends on the length increase and the running time of the UBS Trees algorithm on the smoothness of the path.
Since IPB finds better paths in both regards, both phases run faster and the total running time is faster for small $\epsilon$ despite the larger number of iterations.

With IDB, less than 1\% of all queries were aborted after 250 iterations.
For $\epsilon \geq 0.05$ the running times are also fast enough for interactive responses which makes the algorithm suitable for practical applications.
Solving the problem exactly with IPB blocking is harder and leads to more aborted queries.
However, for sufficiently large $\epsilon$, only few queries fail and total running times are also fast enough for interactive applications.
