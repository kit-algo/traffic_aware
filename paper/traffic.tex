...
On continental-sized road networks with tens of millions of nodes and edges, this preprocessing typically takes a few minutes.
...
The goal is to find a \emph{Hamiltonian} path, i.e. a simple path which traverses every node exactly once.
...
We construct our SSPP instance by replicating the vertices of $G$ into $n$ layers and two additional nodes $s$ and $t$.
...
We now show that no two nodes in the path can correspond to the same vertex and thus that $P = (v_1, \dots, v_n)$ is indeed a Hamiltonian path in $G$.
Suppose for contradiction that $P'_{i,j} = (v^i, \dots, v^j)$ was a subpath of $P'$.
The length $w(P'_{i,j})$ is $(i-j) \cdot (1+\epsilon)$.
Since start and end node correspond to the same vertex, the shortest path with respect to $w$ between these nodes is made up of arcs from $A_{\operatorname{self}}$ and has distance $\dist_w(v^i, v^j) = i-j$.
...
This leads to a \emph{label-correcting} variation of Dijkstra's algorithm with the possibility for multiple labels per node.
A \emph{label} $l$ at a node $v$ consists of a distance from the source $\mathtt{D}_l$, a set of active blocked paths $\mathtt{A}_l$, and a pointer to the parent node and label for efficient reconstruction of the path $P(l) = (s,\dots,v)$ represented by the label.
...
When a node $u$ with a label $l_u$ is popped from the queue and an arc $uv$ is relaxed, we create a new label $l_v$ as follows:
...
As all subpaths of three or more nodes still have a shortest distance of one between their endpoints, we have $\Theta(n^2)$ unique stretch values.
...
In each iteration, we select both endpoints of the considered path and compute distances from and to the endpoints for all nodes on the path.
...
For nodes encountered through recursive shortcut unpacking this might have not happened before.
...
The data includes two live traffic snapshots in the form of OSM node pairs and live speeds for the edge between the nodes.
