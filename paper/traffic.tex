...
Mobile navigation and mapping applications have become ubiquitous.
These applications allow users to query good routes between locations in the road network.
Such queries are usually answered by computing shortest paths on a weighted graph representing the road network with travel times as weights.
...
Considering the current traffic situation for the routing is crucial to suggest good routes to users.
Integrating the current traffic situation comes with its own challenges.
...
The goal is then to find the shortest path with respect to the volatile weights which has a limited uniformly bounded stretch, i.e maximum stretch over all subpaths with respect to the smooth weights.
...
Part of this research has played an important role in enabling modern routing applications.
...
For an extensive overview over available speed-up techniques, we refer to~\cite{bdgmpsww-rptn-16}.

One particularly popular such speed-up technique is Contraction Hierarchies (CH)~\cite{gssv-erlrn-12}.
Our work also makes use of CH.
During preprocessing, additional shortcut edges are inserted into the graph, which skip over unimportant vertices.
...
We therefore utilize the easier to extend classical A* algorithm~\cite{hnr-afbhd-68} with a CH-based heuristic~\cite{strasser_et_al:LIPIcs.SEA.2021.6}.
...
For live traffic, the results were so called \emph{customizable} speed-up techniques which have a two preprocessing phases.
...
CH have with Customizable Contraction Hierarchies (CCH) also been extended to a three-phase setup~\cite{dsw-cch-15}.
...
The shortest smooth path problem was originally introduced in~\cite{dss-tarrn-18}.
The paper includes two CRP-based algorithms for the SSPP.
Our work builds and improves on one of the algorithms presented there.
An important subproblem of SSPP is the computation of UBS values of paths.
...
Section~\ref{sec:prelim} contains notation and recapitulates existing algorithms we build open.
...
In Section~\ref{sec:eval}, we present a thorough evaluation of our algorithm which confirms their applicability.
...
Figure~\ref{fig:perf_profile} depicts performance profiles~\cite{dolan2002benchmarking} for running time and found path length off all queries from Table~\ref{tab:data} combined.
...
For computations that were aborted after 10\,s $\operatorname{obj}(a, q) = \infty$.
...
Our exact version is still about an order of magnitude than the original heuristic implementation.
