...
\author{Tim Zeitz}{Karlsruhe Institute of Technology, Germany}{tim.zeitz@kit.edu}{https://orcid.org/0000-0003-4746-3582}{}%TODO mandatory, please use full name; only 1 author per \author macro; first two parameters are mandatory, other parameters can be empty. Please provide at least the name of the affiliation and the country. The full address is optional. Use additional curly braces to indicate the correct name splitting when the last name consists of multiple name parts.

% \author{Joan R. Public\footnote{Optional footnote, e.g. to mark corresponding author}}{Department of Informatics, Dummy College, [optional: Address], Country}{joanrpublic@dummycollege.org}{[orcid]}{[funding]}

\authorrunning{T. Zeitz} %TODO mandatory. First: Use abbreviated first/middle names. Second (only in severe cases): Use first author plus 'et al.'

\Copyright{Tim Zeitz} %TODO mandatory, please use full first names. LIPIcs license is "CC-BY";  http://creativecommons.org/licenses/by/3.0/
...
\keywords{realistic road networks, route planning, shortest paths, traffic-aware routing, live traffic, uniformly bounded stretch}%TODO mandatory; please add comma-separated list of keywords
...
\supplementdetails[subcategory={Source Code},swhid={swh:1:dir:656ed74654a81353c3de75f0a7a79bbc65ec5818;origin=https://github.com/kit-algo/traffic_aware;visit=swh:1:snp:5723d1737c2eef1db972db07720cf997e6191dc2;anchor=swh:1:rev:c9eaeb0cacdfc064ed4790865b587b66937c86d6}]{Software}{https://github.com/kit-algo/traffic_aware}
%\supplementdetails[linktext={opt. text shown instead of the URL}, cite=DBLP:books/mk/GrayR93, subcategory={Description, Subcategory}, swhid={Software Heritage Identifier}]{General Classification (e.g. Software, Dataset, Model, ...)}{URL to related version} %linktext, cite, and subcategory are optional

%\funding{(Optional) general funding statement \dots}%optional, to capture a funding statement, which applies to all authors. Please enter author specific funding statements as fifth argument of the \author macro.

\acknowledgements{
I want to thank my colleagues for from the scalable algorithms group Christopher Weyand, Marcus Wilhelm and Thomas Bläsius for pointing out a crucial fact on subpath-optimality at our group workshop.
Further, I want to thank my colleague Jonas Sauer for many helpful discussions on algorithmic ideas and proofreading of early drafts of this paper.
I also want to thank the anonymous reviewers for their helpful comments.
Finally, I would like to thank Jakob Bussas who did a proof-of-concept implementation of the ideas presented here for his bachelor's thesis.
}%optional
...
\EventEditors{Christian Schulz and Bora U\c{c}ar}
\EventNoEds{2}
...
In this paper, we settle the complexity of the SSPP and show that it is strongly \textsf{NP}-complete.
We then present practical algorithms to solve the problem on continental-sized road networks both heuristically and exactly.
...
% Another problem is when an alternative route around a traffic jam is suggested but the alternative is congested, too, despite this not being represented in the routing data due to a low-quality traffic feed.
...
In Section~\ref{sec:complexity}, we settle the complexity of the SSPP by proving that it is strongly \textsf{NP}-complete.
...
We denote by $P_{i,j} = (v_i, \dots, v_j), 1 \leq i < j \leq k$ a \emph{subpath} of $P$.
...
To \emph{contract} a vertex means to temporarily remove it from the graph while inserting \emph{shortcut arcs} between more important neighbors to preserve shortest distances among them.
...
The augmented graph has the property that for any two vertices $s$ and $t$, there always exists an \emph{up-down} $st$-path of shortest distance which first uses only arcs from $\gchu$ and then only arcs from $G^{\downarrow}$.
...
The \emph{uniformly bounded stretch} $\ubs_w(P) = \max_{0 \leq i < j \leq k}S_w(P_{i,j})$ indicates the maximum stretch over all subpaths.
...
In this section, we prove that SSPP is strongly \textsf{NP}-complete for any $\epsilon \in \mathbb{Q}^{>0}$.
We define the decision variant of the problem as follows:
An instance $(G, w, w^*, s, t, k)$ of the of $\epsilon$-\textsc{ShortestSmoothPath-Dec} problem admits a feasible solution if and only if there exists a path $P = (s,\dots, t)$ in $G$ with $w^*(P) \leq k$ and $\ubs_w(P) < 1 + \epsilon$.

\begin{theorem}
$\epsilon$-\textsc{ShortestSmoothPath-Dec} is strongly \textsf{NP}-complete for any $\epsilon \in \mathbb{Q}^{>0}$.
\end{theorem}
...
This shows that \textsc{ShortestSmoothPath-Dec} $\in \mathsf{NP}$.

To prove the hardness, we give a reduction from the strongly \textsf{NP}-complete \textsc{HamiltonPath} problem~\cite{gj-ci-79}.
...
This transformation has a running time in $\mathcal{O}(n\cdot(n+m))$.
...
Thus, $\dist_w(P''_{i,j}) \geq (j - i - 1) + (1 + \epsilon)$.
This yields
\[
...
\end{tikzpicture}
\caption{
Example graph where for $\epsilon = 1$ the shortest $\epsilon$-smooth path $(s,v,t)$ is not prefix-optimal. For all arcs except $ut$, the smooth and the volatile weight function are equal. For $ut$, the smooth weight is 1 and the volatile weight 10.
}
...
It might be necessary to keep a label with suboptimal distance from the start as in the example from Figure~\ref{fig:ipb_counterexample} where the label for $(s,v)$ needs to be kept at $v$ despite being longer than $(s,u,v)$.
This leads to a \emph{label-correcting} variation of Dijkstra's algorithm with possibly multiple labels per vertex.
A \emph{label} $l$ at a vertex $v$ consists of a distance $\mathtt{D}_l$ from the source, a set of active blocked paths $\mathtt{A}_l$, and a pointer to the parent vertex and label for efficient reconstruction of the labels' path $P(l) = (s,\dots,v)$.
...
For any active blocked path $B = (\dots, u, x, \dots) \in \mathtt{A}_{l_u}$, we need to check if $x=v$, i.e. $uv$ lies on $B$.
...
The bitset size is determined individually for each vertex by the number of blocked paths the respective vertex lies on.
In our implementation, we use at least one 128-bit integer which suffices for most queries.
Should the number of blocked paths for a vertex exceed 128, we switch to using a dynamically sized array of integers for that vertex.
...
\end{tikzpicture}
\caption{
Example path (solid, black) with shortest path tree from $v_1$ to all vertices on the path (dashed, blue) and reverse shortest path tree from all vertices on the path to $v_9$ (dashed, red).
}
\label{fig:ubs_tree}
\end{figure}
...
Here, we introduce an algorithm which can compute exact UBS values of typical paths with only a few shortest path queries.
We also present a worst-case example where each subpath has a distinct stretch value.
This suggests that achieving subquadratic worst-case running time may not be possible.
...
Thus, the shortest path tree from any vertex is a star with the direct arcs and our algorithm can only advance by a single vertex in each iteration.
...
As all subpaths of three or more vertices still have a shortest distance of one between their endpoints, there are $\Theta(n^2)$ unique stretch values.
...
\begin{algorithm2e}[t]
\KwData{$\mathtt{P}[u]$: parent vertex on the shortest path from $u$ to $t$, as computed by Dijkstra's algorithm on $\gchd$ and an extended Algorithm~\ref{algo:lazy_rphast}}
...
We adjust Algorithm~\ref{algo:lazy_rphast} to continue to maintain these parent pointers during arc relaxation.
Thus, after having called \texttt{ComputeAndMemoizeDist}, we have the shortest path through the CH search space in $G^+$.
Algorithm~\ref{algo:lazy_rphast_path} depicts the routine to efficiently unpack shortcuts on this path and retrieve shortest path trees in the original graph.
We use a bitvector $\mathtt{U}$ (using a clearlist for fast reinitialization) to mark vertices for which the shortest path has already been fully unpacked which is checked before any actual work is performed.
...
Since we are solving an \textsf{NP}-hard problem, this is not very surprising.
...
In fact, \emph{median} running times for 1h queries of all algorithms on all instances are all below 2\,ms. % and the differences for the other query sets become only more extreme.
...
In this paper, we studied the shortest smooth path problem and proved its \textsf{NP}-completeness.
